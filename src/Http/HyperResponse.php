...
<body style="background: #ef4444; color: white; padding: 20px; font-family: monospace;">
    ...
    /**
     * Process JSON query array navigation
     *
     ...
     */
    private function processJsonNavigation(array $queries, array $options): string
    {
        // Build query string from array
        $queryString = $this->buildQueryString($queries);

        // Start with current path
        $currentPath = request()->getPathInfo();
        $baseUrl = $queryString ? "{$currentPath}?{$queryString}" : $currentPath;

        // Apply merge logic if specified
        if ($this->shouldApplyMerge($options)) {
            return $this->mergeQueryParameters($baseUrl, $options);
        }
    ...
    /**
     * Process traditional string URL navigation
     *
     ...
     * @return string Final URL
     */
    private function processStringNavigation(string $url, array $options): string
    {
        // Handle additional queries option
        if (!empty($options['queries']) && is_array($options['queries'])) {
            /** @var array<string, mixed> $queries */
            $queries = $options['queries'];
            $url = $this->mergeQueriesIntoUrl($url, $queries);
        }

        // Apply merge logic if specified
        if ($this->shouldApplyMerge($options)) {
            return $this->mergeQueryParameters($url, $options);
        }
     ...
     * @param array<string, mixed> $options Navigation options
     */
    private function shouldApplyMerge(array $options): bool
    {
        // Explicit merge setting takes precedence
        if (isset($options['merge'])) {
            return (bool) $options['merge'];
        }
     ...
     *
     * @param string $url Original URL
     * @param array<string, mixed> $additionalQueries Queries to merge
     *
     * @return string Updated URL
     */
    private function mergeQueriesIntoUrl(string $url, array $additionalQueries): string
    {
        $parsedUrl = parse_url($url);

        // Extract existing query parameters
        parse_str($parsedUrl['query'] ?? '', $existingQueries);

        // Merge with additional queries
        $mergedQueries = array_merge($existingQueries, $additionalQueries);

        // Remove null/empty values
        $mergedQueries = array_filter($mergedQueries, function ($value) {
            return $value !== null && $value !== '';
        });

        // Rebuild URL
        $path = $parsedUrl['path'] ?? '/';
        $queryString = http_build_query($mergedQueries);

        return $queryString ? "{$path}?{$queryString}" : $path;
    }

    /**
     * Merge query parameters with current request - consistent with frontend
     *
     * @param string $url Target URL
     * @param array<string, mixed> $options Merge options (only, except)
     *
     * @return string Final URL
     */
    private function mergeQueryParameters(string $url, array $options): string
    {
        $currentQueries = request()->query();

        // Parse target URL
        $urlParts = parse_url($url);
        $targetPath = $urlParts['path'] ?? request()->getPathInfo();

        parse_str($urlParts['query'] ?? '', $targetQueries);

        // Filter current queries based on only/except options
        $filteredCurrentQueries = $this->filterQueries($currentQueries, $options);

        // Merge queries (target overrides current)
        $mergedQueries = array_merge($filteredCurrentQueries, $targetQueries);

        // Remove null/empty values (they clear parameters)
        $mergedQueries = array_filter($mergedQueries, function ($value) {
            return $value !== null && $value !== '';
        });
    ...
    /**
     * Filter queries based on only/except options
     *
     * @param array<string, mixed> $queries Original queries
     * @param array<string, mixed> $options Filter options
     *
     * @return array<string, mixed> Filtered queries
     */
    private function filterQueries(array $queries, array $options): array
    {
        if (!empty($options['only']) && is_array($options['only'])) {
            ...
            return array_intersect_key($queries, array_flip($only));
        }

        if (!empty($options['except']) && is_array($options['except'])) {
            // Preserve all except specified parameters
            /** @var array<int, string> $except */
            $except = $options['except'];

            return array_diff_key($queries, array_flip($except));
        }
